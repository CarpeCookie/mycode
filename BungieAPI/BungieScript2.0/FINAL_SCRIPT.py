...
#These values are tied to a specific item instance.  If I change the shader on the item, the hash will stay the same, even though the item will look different.  I don't know where the info on the specific shader is located, so I can't use that as the value.  This is really just to show I can check for specific item sets.
